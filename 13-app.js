...
async function boot() {
  try {
    ...
    blog('fetching g1 asset pack (meshes + onnx policy)');
    const zipBuf = new Uint8Array(await (await fetch('./g1.zip')).arrayBuffer());
    ...
    for (const [path, content] of Object.entries(files)) {
      ...
      mujoco.FS.writeFile('/working/' + path, content);
      if (path.endsWith('.STL')) meshCount++;
    }
    blog('asset pack unpacked: ' + meshCount + ' meshes + robot MJCF + policy.onnx');

    blog('loading onnx policy (MLP 480 -> 29, unitree rl gym velocity)');
    ort.env.wasm.wasmPaths = './';
    ort.env.wasm.numThreads = 1;
    const onnxBuf = files['policy.onnx'];
